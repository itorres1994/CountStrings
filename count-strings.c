...
int count_occurrencesH(char *arr, char *find, long unsigned int length, int count) {
  int i = 0;
  int j = 0;
  while(*arr != '\0') {
     if(*arr == find[j]) {
...
void count_occurrences (int n, FILE *file, Entry *entries) {
  ...
  int i = 0;
  //int index = 0;
  unsigned char spoofArr[LINE_MAX_CHARS];
  char *ch;
  while(fgets(spoofArr,LINE_MAX_CHARS,file) != NULL) {
    for(; i < n; i++) {
       ch = entries[i].string;
       entries[i].count = count_occurrencesH(spoofArr,ch,strlen(ch),entries[i].count);
    }
    i = 0;
  }
}
